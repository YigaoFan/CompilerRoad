...
*** TODO Generate parse code[10/22] DEADLINE: <2025-03-17 Mon>
...
- [X] complete a Pratt parser demo <2025-03-20 Thu> hard code token type style
...
- [ ] check use of private lex rule symbol(non-token type) in parse rules. Too many rules reference the private token. Move them into parse rule? Use checker to check which parse rule references private token type
  - [ ] literal-identifier. Promote them to parse rules for more flexibility. <2025-03-23 Sun>
- [ ] terminalXXX will change if change vba.abnf file, it needs to generate code to fix it.
- [ ] why not load constant-expression grammar? Because parseTable doesn't have expression token type to store in it. Need to fix it in GrammarSet.
